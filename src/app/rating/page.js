...
export default function DoctorDetail() {
  ...
  const fetchDoctorDetails = async (userId, token) => {
    setLoading(true);
    try {
      const data = await apiRequest(ENDPOINTS.GET_CAREGIVER(userId));
      if (!data || data.userType !== 'CAREGIVER') {
      ...
      <div className="flex items-center">
        {[...Array(5)].map((_, i) => (
          <svg
            key={i}
            className={`h-5 w-5 ${
              i < fullStars ? 'text-yellow-400' : i === fullStars && hasHalfStar ? 'text-yellow-300' : 'text-gray-300'
            }`}
            fill="currentColor"
            viewBox="0 0 20 20"
          >
    ...
    <div className="py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-lg text-gray-600">How your patients rate you</p>
        </div>
              ...
              <div key={review.id} className="bg-gray-50 p-4 rounded-lg shadow-sm">
                <div className="flex items-center mb-2">
                  <span className="font-semibold text-gray-800">
                    {review.userId ? `Pacillian ${review.userId}` : 'Anonymous'}
                  </span>
